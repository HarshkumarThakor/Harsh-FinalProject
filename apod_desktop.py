...
History:
  ...
  2022-03-11  Harshkumar Initial creation
"""
...
async def main():  #async
    ...
    image_msg = await download_apod_image(image_url)
    #print(image_msg)
    ...
    image_size = -1 # TODO
    image_path = image_dir_path+apod_info_dict["title"]+".jpg"#get_image_path(image_url, image_dir_path)
    ...
    if not image_already_in_db(db_path, image_sha256):
        ...
        add_image_to_db(apod_date, db_path, apod_info_dict["title"] + ".jpg", image_path, image_size, image_sha256,
                        image_url)
...
def get_apod_info(date):
    ...
    """
    apod_url = "https://api.nasa.gov/planetary/apod?api_key=jqF2TjjJcVqiPnB7gLZcxgvO0dTAYX12LdMKfd1h"
    apod_data = request.get(apod_url,params={"date": date.strftime('%Y-%m-%d')})
    if apod_data.status_code == 200:
        try:
            print("Downloading...")
            body = apod_data.content
...
async def download_apod_image(image_url): #Ensure this function returns a promise.
    """
    ...
    """
    image_data = request.get(image_url)
...
def create_image_db(db_path):
    ...
    create_IOD_database = """ CREATE TABLE IF NOT EXISTS 'ImageofTheDay' (
                                        'Date' text NOT NULL,
                                        'DownloadedDate'  text NOT NULL,
                                        'Name' text NOT NULL,
                                        'Size'  text NOT NULL,
                                        'SHA_256Hash' text NOT NULL,
                                        'Path' text NOT NULL,
                                        'URL' text NOT NULL,
                                        PRIMARY KEY ('Name')
                                    );"""
    cursor.execute(create_IOD_database)
    return  # TODO

def add_image_to_db(date,db_path,file_name, image_path, image_size, image_sha256, image_url):
    """
    ...
    connection = sqlite3.connect(db_path)
    cursor = connection.cursor()
    addData = """INSERT INTO 'ImageofTheDay' (
                                'Date',
                                'DownloadedDate',
                                'Name', 
                                'Size', 
    ...
    IOD_data = (date,
                ...
                image_path,
                image_url)
    cursor.execute(addData, IOD_data)
    connection.commit()
    connection.close()
    return #TODO
...
def image_already_in_db(db_path, image_sha256):
    ...
    connection = sqlite3.connect(db_path)
    cursor = connection.cursor()
    addData = "SELECT * FROM ImageofTheDay WHERE SHA_256Hash = ?;"
    cursor.execute(addData,(image_sha256,))
    row = cursor.fetchone()
    connection.commit()
    connection.close()
    if row is None:
